...
class Invaders(GameApp):
    ...
    def start(self):
        ...
        self._text = GLabel(text = "Press 'S' to Play", x = 400,
         y = 350, font_size = 60, font_name ='Arcade.ttf')
    ...
    def update(self,dt):
        ...
        """
        # We referred to state.py from the sample code to write this.
        self.determinestate()
        self.updatewave(dt)
        self.gamestop()
    ...
    def determinestate(self):
        """
        This method is responsible for determining and changing game states.
        """
    ...
    def gamestop(self):
        """
        This method handles the game when it is paused or ended because of a
        player win.
        """
        if self._wave != None:
            if self._wave.getDead():
                ...
                if self._wave.getLives() > 0:
                    self._text = GLabel(text = "Press 'R' to Continue", x = 400,
                     y = 350, font_size = 60, font_name ='Arcade.ttf',\
                      linecolor = 'blue')
            if self._wave.getLives() == 0 or self._wave.getDipped():
                self._state = STATE_COMPLETE
                self._text = GLabel(text = "You Lost!", x = 400,
                 y = 350, font_size = 80, font_name ='Arcade.ttf', \
                 linecolor = 'red')
            if self._wave.getWin():
                self._state = STATE_COMPLETE
                self._text = GLabel(text = "You Won!", x = 400,
                 y = 350, font_size = 80, font_name ='Arcade.ttf', \
                 linecolor = 'green')
    ...
    def updatewave(self,dt):
        """
        If the state is active, the subcontroller for a single wave is created.

        Parameter dt: The time in seconds since last update
        Precondition: dt is a number (int or float)
        """
        if self._state == STATE_ACTIVE and self._state != STATE_PAUSED \
        and self._state != STATE_COMPLETE:
