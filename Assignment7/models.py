...
class Ship(GSprite):
    ...
    def __init__(self):
        ...
        """
        super().__init__(x= GAME_WIDTH/2, bottom=SHIP_BOTTOM, \
    ...
    def collides(self,bolt):
        ...
        return self.contains((bolt.right,bolt.bottom)) or \
            self.contains((bolt.left,bolt.bottom))

    # COROUTINE METHOD TO ANIMATE THE SHIP
    def animates(self, dt):
        """
        ...
        """
        super().__init__(x = self.x, y = self.y, width = SHIP_WIDTH, height = \
            SHIP_HEIGHT, source='ship-strip.png',format=(2,4), frame = 1)
        ...
        while animating:
            dt = (yield)
            value = (time/DEATH_SPEED)
            amount = value * 7
            self.frame = round(amount) + 1
            time = time + dt
            if time > DEATH_SPEED:
                print(time)
                animating = False
...
        Parameter y: the y-coordinate of an alien
        Precondition: y is an int
        Parameter source: an image file in the Images folder
        Precondition: source is an image file in the Images folder
        """
    ...
    def collides(self,bolt):
        ...
        return self.contains((bolt.left, bolt.top)) or \
            self.contains((bolt.right,bolt.top))
...
        Precondition: velocity is an int
        """
        super().__init__(x = x, y = y, width = \
            BOLT_WIDTH, height = BOLT_HEIGHT, fillcolor = 'black')
