...
class Cluster(object):
    ...
    def getContents(self):
        ...
        # IMPLEMENT ME
        result = []
        for x in self._indices: #x is index of a point
            result.append(self._dataset.getPoint(x))
        return result
    ...
    def distance(self, point):
        ...
        for x in range(len(point)):
            result.append((self._centroid[x] - point[x])**2)
    ...
    def update(self):
        ...
        result = [0] * self._dataset.getDimension()
        for x in self.getIndices():
            point = self._dataset.getPoint(x)
            for pos in range(len(point)):
                result[pos] = result[pos] + point[pos]
        for pos in range(len(point)):
            result[pos] = result[pos]/len(self.getIndices())

        x = numpy.allclose(self._centroid, result)
        self._centroid = tuple(result)
